...
var FileDAO = function(settings, mongo, utils) {
    ...
    this.save = function(file, callback) {
        mongo.files.save(file, function(error, data) {
                ...
                fs.stat(path, function(error, stat) {
                    if (!error && stat.isDirectory()) {
                        dao.index(dao, {
                            ...
                            if (!error && data._id) {
                                dao.walk(dao, path, data._id, function(dao) {
                                    dao.stop('indexing success');
                                });
                            } else {
                                dao.stop('could not index files'); //save error
                            }
                        });
                    } else {
                        dao.stop('could not stat files'); //save error
                    }
                });
            });
                    ...
                    if (!error && stat) {
                        if (stat.isDirectory()) {
                            dao.index(dao, {
                                ...
                                if (!error && data._id) {
                                    dao.walk(dao, _path, data._id, function() {
                                        next();
                                    });
                                } else {
                                    next();
